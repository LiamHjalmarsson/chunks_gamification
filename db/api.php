...
if ($method_action !== "GET_login") {
    ...
    if (!$is_teacher) {
        ...
        if ( ($method === "POST" || $method === "PATCH") && ($action !== "users_units" && $action !== "quiz_answer" && $action !== "badges" && $action !== "userBadges" && $action !== "userRank" && $action !== "ranking" && $action !== "units_quizs_questions" && $action !== "streak")) {
            $response_function([
                "code" => 400,
